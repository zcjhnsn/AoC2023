...
private class CommsModule {
    ...
    static func make(from string: String) -> CommsModule {
...
private final class FlipFlop: CommsModule {
...
private final class Conjunction: CommsModule {
...
private final class Broadcaster: CommsModule {
    override func receive(_ pulse: Pulse, from name: String) -> [Message] {
        destinations.map { Message(pulse: pulse, from: self.name, destination: $0) }
    }
}

private class Output: CommsModule {}

final class Day20: AOCDay {
    
    private let modules: [String: CommsModule]
    
    init(input: String) {
        let rawModules = input.lines.map { CommsModule.make(from: $0) }
        ...
        for module in rawModules {
            for destination in module.destinations {
                if modules[destination] == nil {
                    modules[destination] = Output(name: destination, destinations: [])
                }
            }
        }
    ...
    func part2() -> Int {
        ...
        // find it's input conjunctions so we can monitor them 
